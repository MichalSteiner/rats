"""
This module provides functions for plotting Radial Velocity (RV) plots during transit and other related plots.

Functions:
- plot_RV(spectrum_list: sp.SpectrumList):
    Plots all RV from the spectrum list header.

- _plot_posterior(data_chain: list, type: str):
    Plots the posterior distribution for the given data chain and type.

- plot_posterior_local_CCF(all_data_chain: list, data_raw_A: sp.SpectrumList):
    Plots the posterior distribution for local CCF.

- model_velocity(CCF: sp.Spectrum1D, system_parameters, revolutions_result):
    Models the velocity for the given CCF and system parameters.

- get_gaussian_fit(CCF, system_parameters, result_Revolutions):
    Gets the Gaussian fit for the given CCF and system parameters.

- plot_Gaussian_values(data_chain_list: list, CCF_intrinsic: sp.SpectrumList, system_parameters, CCF_intrinsic_full: sp.SpectrumList):
    Plots the Gaussian values for the given data chain list and CCF intrinsic.

- _get_residuals(CCF_intrinsic, system_parameters, revolutions_result):
    Gets the residuals for the given CCF intrinsic and system parameters.

- plot_CCF_night(CCF_list, night_num, system_parameters, ax, cmap='magma_r', vmin=0.2, vmax=1.2):
    Helper function to plot CCF data for a single night.

- plot_residual_CCF_night(CCF_list, night_num, system_parameters, revolutions_result, ax, cmap='RdBu_r', vmin=-0.15, vmax=0.15):
    Helper function to plot CCF residual data for a single night.

- plot_instrinsic_CCF(CCF_intrinsic, system_parameters, fig: plt.Figure | None = None, axs: plt.Axes | None | np.ndarray = None):
    Plots the intrinsic CCF for the given CCF intrinsic and system parameters.

- plot_instrinsic_residuals_CCF(CCF_intrinsic, system_parameters, revolutions_result, fig: plt.Figure | None = None, axs: plt.Axes | None | np.ndarray = None):
    Plots the intrinsic residuals CCF for the given CCF intrinsic and system parameters.

- SpectraViewer:
    Class for viewing spectra.

- RevolutionsSlides:
    Class for creating slides for revolutions.
"""
...
def plot_RV(spectrum_list: sp.SpectrumList):
    ...
    for ind, night in enumerate(np.unique([item.meta['Night'] for item in spectrum_list])):
        ...
        rv = [item.meta['header']['HIERARCH ESO QC CCF RV']
              for item in sublist]
        rv_error = [item.meta['header']['HIERARCH ESO QC CCF RV ERROR']
                    for item in sublist]
            ...
            case 'rvcenter':
                az.plot_dist(
                    ...
                    color=sns.color_palette(
                        "dark")[test_CCF[ind].meta['Night_num']-1],
...
def plot_posterior_local_CCF(all_data_chain: list,
    ...
    if divmod(len(all_data_chain), 10)[1] != 0:
        fig, axs = plt.subplots(divmod(len(all_data_chain), 10)[
                                0]+1, 10, sharex=False, sharey=False, figsize=fig_size, squeeze=False)
    else:
        fig, axs = plt.subplots(divmod(len(all_data_chain), 10)[
                                0], 10, sharex=False, sharey=False, figsize=fig_size, squeeze=False)
    ...
    for ind, test_chain in enumerate(all_data_chain):
        ...
        axs[divmod(ind, 10)].axvline(test_chain.posterior.rvcenter.median(
        ), color='black', ls='--', label='_nolegend_')
        axs[divmod(ind, 10)].legend()
        axs[divmod(ind, 10)].label_outer(remove_inner_ticks=True)
        axs[divmod(ind, 10)].set_yticks([])
        axs[divmod(ind, 10)].set_xticks([])
    ...
    for ind, test_chain in enumerate(all_data_chain):
        az.plot_dist(test_chain.posterior.contrast,
                     ...
                     color=sns.color_palette(
                         "dark")[test_CCF[ind].meta['Night_num']-1],
                     label=test_CCF[ind].meta['Spec_num'])
        axs[divmod(ind, 10)].axvline(test_chain.posterior.contrast.median(
        ), color='black', ls='--', label='_nolegend_')
        axs[divmod(ind, 10)].legend()
        axs[divmod(ind, 10)].label_outer(remove_inner_ticks=True)
        axs[divmod(ind, 10)].set_yticks([])
    ...
    for ind, test_chain in enumerate(all_data_chain):
        ...
        axs[divmod(ind, 10)].axvline(test_chain.posterior.fwhm.median(),
                                     color='black', ls='--', label='_nolegend_')
        axs[divmod(ind, 10)].legend()
        axs[divmod(ind, 10)].label_outer(remove_inner_ticks=True)
        axs[divmod(ind, 10)].set_yticks([])
...
def model_velocity(CCF: sp.Spectrum1D,
    ...
    if 'contrast' in revolutions_result.posterior.data_vars:
        ...
        contrast = revolutions_result.posterior[f"contrast_night{CCF.meta['Night']}_0"].mean(
        ).to_numpy()
    ...
    if 'fwhm' in revolutions_result.posterior.data_vars:
        ...
        fwhm = revolutions_result.posterior[f"fwhm_night{CCF.meta['Night']}_0"].mean(
        ).to_numpy()
    ...
    y_p = aRs * np.cos(2*np.pi * phase) * \
        np.cos(system_parameters.Planet.inclination.data/180*np.pi)

    x_perpendicular = x_p * \
        np.cos(obliquity/180*np.pi) - y_p * np.sin(obliquity/180*np.pi)
    y_perpendicular = x_p * \
        np.sin(obliquity/180*np.pi) - y_p * np.cos(obliquity/180*np.pi)

    local_stellar_velocity = x_perpendicular * veqsini

    expected = (-contrast * np.exp(-((CCF.spectral_axis.value -
                local_stellar_velocity)**2)/(2*((fwhm/2.35482)**2))) + 1)
    return expected
...
class CompareIntr_vs_RevolutionsFit:
    def __init__(self, CCF_list, system_parameters, result):
        ...
        self.text_artist = self.fig.text(
            0.05, 0.05, '', transform=self.fig.transFigure)
    ...
    def update_plot(self):
        ...
        self.ax.errorbar(CCF.spectral_axis, CCF.flux,
                         CCF.uncertainty.array, fmt='.')
...
class CompareIntr_vs_RevolutionsFit_vs_localfit:
    ...
    def update_plot(self):
        ...
        # Update text (example: showing current spectrum info)
        self.update_text(
            f'Spectrum index: {self.current_index}\n Night: {CCF.meta.get("Night", "N/A")} \n Revolutions fit: {CCF.meta.get("Revolutions", "N/A")}')
...
def plot_Gaussian_values(data_chain_list: list,
    ...
    considered_indices = [CCF.meta['Spec_num']
                          for CCF in CCF_intrinsic_full if CCF.meta['Transit_partial']]
    ...
    for CCF, data_chain in zip(in_transit, data_chain_list):
        ...
        mu = abs(system_parameters.Planet.a_rs_ratio *
                 np.sin(2*np.pi * CCF.meta['Phase'].data))
        ...
        contrast_error = np.asarray([
            ...
            ) - az.hdi(data_chain.posterior.contrast, hdi_prob=0.683)['contrast'].to_numpy())[0],
            (- data_chain.posterior.contrast.median().to_numpy() +
             az.hdi(data_chain.posterior.contrast, hdi_prob=0.683)['contrast'].to_numpy())[1]
        ]).reshape(2, 1)
        fwhm_error = np.asarray([
            (data_chain.posterior.fwhm.median().to_numpy(
            ) - az.hdi(data_chain.posterior.fwhm, hdi_prob=0.683)['fwhm'].to_numpy())[0],
            (- data_chain.posterior.fwhm.median().to_numpy() +
             az.hdi(data_chain.posterior.fwhm, hdi_prob=0.683)['fwhm'].to_numpy())[1]
        ]).reshape(2, 1)

        rvcenter_error = np.asarray([
            (data_chain.posterior.rvcenter.median().to_numpy(
            ) - az.hdi(data_chain.posterior.rvcenter, hdi_prob=0.683)['rvcenter'].to_numpy())[0],
            (- data_chain.posterior.rvcenter.median().to_numpy() +
             az.hdi(data_chain.posterior.rvcenter, hdi_prob=0.683)['rvcenter'].to_numpy())[1]
        ]).reshape(2, 1)
    ...
    axs[2, 0].fill_between(phase_model, rv_loc_value-rv_loc_error,
                           rv_loc_value+rv_loc_error, color='green', alpha=0.2)
    ...
    axs[2, 0].plot(phase_model, rv_loc_value, color='black')
    axs[2, 0].fill_between(phase_model, rv_loc_value-rv_loc_error,
                           rv_loc_value+rv_loc_error, color='red', alpha=0.2)
...
def _get_residuals(CCF_intrinsic,
    ...
    for CCF in CCF_intrinsic:
        residual_flux = CCF.flux.value - \
            model_velocity(CCF, system_parameters, revolutions_result)
...
def plot_CCF_night(CCF_list, night_num, system_parameters, ax, cmap='magma_r', vmin=0.2, vmax=1.2):
    ...
    CCF_night = sm.get_sublist(
        CCF_night, 'Transit_partial', True, mode='equal')
...
def plot_residual_CCF_night(CCF_list, night_num, system_parameters, revolutions_result, ax,
    ...
    CCF_residual = _get_residuals(
        CCF_night, system_parameters, revolutions_result)
...
def plot_instrinsic_CCF(CCF_intrinsic,
    ...
    num_night = len(np.unique([CCF.meta['Night_num']
                    for CCF in CCF_intrinsic]))
...
def plot_instrinsic_residuals_CCF(CCF_intrinsic,
                                  ...
                                  axs: plt.Axes | None | np.ndarray = None):
    """
    Plots the intrinsic Cross-Correlation Function (CCF) and its residuals for multiple nights.
    Parameters:
    -----------
    CCF_intrinsic : list
        List of intrinsic CCFs, each containing metadata including 'Night_num'.
    system_parameters : object
        Object containing system parameters such as Planet and Star properties.
    revolutions_result : object
        Object containing the results of the revolutions, including posterior distributions.
    fig : plt.Figure, optional
        Matplotlib Figure object. If None, a new figure will be created.
    axs : plt.Axes, np.ndarray, or None, optional
        Matplotlib Axes object or array of Axes. If None, new axes will be created.
    Returns:
    --------
    fig : plt.Figure
        The figure object containing the plots.
    axs : np.ndarray
        Array of Axes objects containing the subplots.
    """
    ...
    if fig is None:
        fig, axs = plt.subplots(num_night, 2, figsize=(
            16, 4*num_night), squeeze=False)

    # Plot left column (intrinsic CCF)
    pcm_list_left = []
    for ni in range(num_night):
        pcm = plot_CCF_night(CCF_intrinsic, ni+1,
                             system_parameters, axs[ni, 0])
        ...
        phases = np.linspace(axs[ni, 0].get_ylim()[0],
                             axs[ni, 0].get_ylim()[1], 1000)
        x_p = aRs * np.sin(2*np.pi * phases)  # type: ignore
        y_p = aRs * np.cos(2*np.pi * phases) * np.cos(
            system_parameters.Planet.inclination.data / 180*np.pi)  # type: ignore
        x_perpendicular = x_p * np.cos(obliquity/180*np.pi) - y_p * \
            np.sin(obliquity/180*np.pi)  # type: ignore

        local_stellar_velocity = x_perpendicular * veqsini

        axs[ni, 0].plot(local_stellar_velocity, phases,
    ...
    for ni in range(num_night):
        ...
        axs[ni, 1].plot(local_stellar_velocity, phases,
                        color='black', ls='dotted')
...
class SpectraViewer:
    """
    A class to visualize and navigate through a list of Cross-Correlation Functions (CCFs) spectra.

    Attributes:
    -----------
    CCF_list : list
        A list of CCF objects containing spectral data.
    system_parameters : dict
        A dictionary containing system parameters.
    current_index : int
        The current index of the spectrum being displayed.
    n_spectra : int
        The total number of spectra in the CCF_list.
    fig : matplotlib.figure.Figure
        The main figure for plotting.
    ax : matplotlib.axes._subplots.AxesSubplot
        The main axis for plotting.
    ax_prev : matplotlib.axes._axes.Axes
        The axis for the 'Previous' button.
    ax_next : matplotlib.axes._axes.Axes
        The axis for the 'Next' button.
    btn_prev : matplotlib.widgets.Button
        The 'Previous' button.
    btn_next : matplotlib.widgets.Button
        The 'Next' button.
    x_min : float
        The minimum value of the spectral axis across all spectra.
    x_max : float
        The maximum value of the spectral axis across all spectra.
    text_artist : matplotlib.text.Text
        The text artist for displaying additional information.

    Methods:
    --------
    __init__(self, CCF_list, system_parameters):
        Initializes the SpectraViewer with a list of CCFs and system parameters.
    update_text(self, new_text):
        Updates the text displayed on the figure.
    update_plot(self):
        Updates the plot with the current spectrum and its metadata.
    next_spectrum(self, event):
        Advances to the next spectrum in the list.
    previous_spectrum(self, event):
        Goes back to the previous spectrum in the list.
    """

    def __init__(self, CCF_list: sp.SpectrumList, system_parameters):
        self.CCF_list = CCF_list
        self.system_parameters = system_parameters
        self.current_index = 0
        self.n_spectra = len(CCF_list)
    ...
    def update_text(self, new_text: str):
        """
        Update the text displayed by the text artist.
    ...
    def update_plot(self):
        """
        Update the plot with the current CCF (Cross-Correlation Function) data.

        This method clears the current plot, plots the CCF with error bars, sets the plot limits,
        updates the background color based on transit status, updates the title, and refreshes the plot.

        The background color is set based on the transit status:
            - Green (#A8DAB5) if both Transit_partial and Transit_full are True.
            - Yellow (#FFF59D) if Transit_partial is True and Transit_full is False.
            - Pink (#F8BBD0) if both Transit_partial and Transit_full are False.

        The title is updated to show the current spectrum index out of the total number of spectra.

        The text is updated to show the current spectrum index, night, and revolutions fit.

        Attributes:
            self.ax (matplotlib.axes.Axes): The axes object to plot on.
            self.CCF_list (list): List of CCF objects containing spectral data.
            self.current_index (int): The index of the current CCF in the list.
            self.x_min (float): The minimum x-axis limit for the plot.
            self.x_max (float): The maximum x-axis limit for the plot.
            self.n_spectra (int): The total number of spectra.
            self.fig (matplotlib.figure.Figure): The figure object containing the plot.
    ...
    def next_spectrum(self, event: mpl.backend_bases.Event):
        """
        Advances to the next spectrum in the sequence and updates the plot.

        Parameters:
        event (matplotlib.backend_bases.Event): The event that triggered this method, typically a button press or key press.
...
class RevolutionsSlides:
    ...
    def plot_dopplergram(self, ax, resolution=1000):
        ...
        if y_planet < 0:
            ...
            planet_mask = ((xx - x_planet)**2 + (yy - y_planet)
                           ** 2 <= planet_radius**2)
        ...
        # Set limits
        ax.set_ylim(-1.2, 1.2)
        ax.set_xlim(aspect_ratio * ax.get_ylim()
                    [0], aspect_ratio * ax.get_ylim()[1])
    ...
    def update_plot(self):
        ...
        self.axs[1].errorbar(CCF.spectral_axis.value,
                             CCF.flux.value, CCF.uncertainty.array, fmt='.')
        ...
        vsini = self.star_params['vsini']
        self.axs[1].axvline(x=-vsini, color='darkblue',
                            linestyle='--', alpha=0.8)
        self.axs[1].axvline(x=0, color='black', linestyle='--', alpha=0.8)
        self.axs[1].axvline(x=vsini, color='darkred',
                            linestyle='--', alpha=0.8)

        self.axs[1].set_title(
            f'Spectrum {self.current_index}/{self.n_spectra -1}')
