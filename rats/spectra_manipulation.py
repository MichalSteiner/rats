...
def _sigma_clipping_spectrum(spectrum: sp.Spectrum1D | sp.SpectrumCollection,
    ...
    """
    Sigma clipping of single spectrum. The rolling window is of size 500 by default.
    ...
    window_size : int, optional
        Size of the rolling window, by default 500.
    ...
    if isinstance(spectrum, sp.Spectrum1D):
    ...
    elif isinstance(spectrum, sp.SpectrumCollection):
        raise NotImplementedError(
            'Please test whether this implementation works.')
        ...
        raise TypeError('Unsupported spectrum type.')
    return new_spectrum
...
def mask_flux_below_threshold_in_list(spectrum_list: sp.SpectrumList,
    ...
    """
    Masks flux below a certain threshold in the spectrum list.

    Parameters
    ----------
    spectrum_list : sp.SpectrumList
        Spectrum list for which to mask flux below the threshold.
    threshold : float, optional
        Threshold below which to mask the flux, by default 100.
    polynomial_order : int, optional
        Order of the polynomial to fit for the threshold, by default 6.
    ...
    -------
    sp.SpectrumList
        Spectrum list with flux below the threshold masked.
    """
    new_spectrum_list = sp.SpectrumList()
...
def execute_mask_in_list(spectrum_list: sp.SpectrumList) -> sp.SpectrumList:
    """
    Executes the mask for each spectrum in the spectrum list.

    Parameters
    ----------
    spectrum_list : sp.SpectrumList
        Spectrum list for which to execute the mask.

    Returns
    -------
    sp.SpectrumList
        Spectrum list with executed masks.
    """
...
# %% extract_sgl_order
def extract_single_order(spectrum_collection: sp.SpectrumCollection,
                         order: int) -> sp.SpectrumList:
    """
    Extract specific order from spectrum collection.

    Parameters
    ----------
    spectrum_collection : sp.SpectrumCollection
        Spectrum collection from which to extract the order.
    order : int
        Order to extract. Note that the order is 0-indexed.

    Returns
    -------
    sp.SpectrumList
        Spectrum list with the extracted order.
    """
    spectrum_list = sp.SpectrumList()
    for spectrum in spectrum_collection:
        if spectrum.meta is None:
            raise TypeError(f"Meta attribute is None for item: {spectrum}")
        else:
            spectrum.meta.update({'S_N': spectrum.meta['S_N_all'][order]})
        spectrum_list.append(spectrum[order])
    return spectrum_list

# %% extract_orders_ESPRESSO
def extract_orders_ESPRESSO(spectrum_list: sp.SpectrumCollection,
                            orders: list[int]) -> sp.SpectrumList:
    """
    Extract specific orders from spectrum collection (For ESPRESSO double orders).

    Parameters
    ----------
    spectrum_list : sp.SpectrumCollection
        Spectrum collection from which to extract the orders.
    orders : list[int]
        List of orders to extract. WARNING: ESPRESSO orders are from 1 to 170, while sp.SpectrumCollection works from 0 to 169.

    Returns
    -------
    sp.SpectrumList
        Spectrum list with the extracted orders.
    """
    spec_list = sp.SpectrumList()
    for spectrum in spectrum_list:
        if spectrum.meta is None:
        ...
        for order in orders:
            spectrum.meta.update({'S_N': spectrum.meta['S_N_all'][order]})
            spec_list.append(spectrum[order-1])
    return spec_list

# %% determine_rest_frame
def _determine_rest_frame(spectrum: sp.Spectrum1D | sp.SpectrumCollection):
    """
    Determine rest frame for given spectrum based on the velocities corrected.

    Parameters
    ----------
    spectrum : sp.Spectrum1D | sp.SpectrumCollection
        Spectrum for which to determine the rest frame.

    Changes
    -------
    meta['RF'] : str
        Changed to estimated rest frame ('Earth', 'Barycenter_Sol', 'Sun', 'Barycenter_Star', 'Star', 'Planet').
    """
    if spectrum.meta is None:
            raise TypeError(f"Meta attribute is None for item: {spectrum}")
    ...
    if test_meta == [True, False, False, False]:
        ...
        spectrum.meta['RF'] = 'Barycenter_Sol'
...
def print_info_spectrum_list(spectrum_list: sp.SpectrumList) -> None:
    ...
    None
    """
    logger.print('='*30) # type: ignore
    for item in spectrum_list:
        logger.print( # type: ignore
            f'Night: {item.meta["Night"]} | #Night: {item.meta["Night_num"]} | #Spectrum_in_night: {item.meta["Night_spec_num"]} | #Spectrum: {item.meta["Spec_num"]}')
    logger.print('='*30) # type: ignore
    return None

# %% tie_wlg
def tie_wavelengths(model: astropy.modeling.models.Gaussian1D) -> astropy.modeling.models.Gaussian1D: # type: ignore
    """
    ...
    model : astropy.modeling.models.Gaussian1D
        Gaussian model to tie wavelengths for.

    Returns
    -------
    model : astropy.modeling.models.Gaussian1D
        Model with tied wavelengths.
    """
    # TODO Move to spectra_manipulation_routines.models
    model.mean_1 = model.mean_0 + (5895.924 - 5889.950)
    return model
...
def binning_spectrum(spectrum: sp.Spectrum1D,
                     bin_factor: int = 10,
                     as_spectrum: bool = False) -> sp.Spectrum1D | tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    x = sci.stats.binned_statistic(
        spectrum.spectral_axis, spectrum.spectral_axis, statistic=np.nanmean, bins=num_bins)

    y = sci.stats.binned_statistic(
        spectrum.spectral_axis, spectrum.flux, statistic=np.nanmean, bins=num_bins)

    y_err = sci.stats.binned_statistic(
        spectrum.spectral_axis, spectrum.uncertainty.array, statistic=smcalc._error_bin, bins=num_bins)
    
    if as_spectrum:
        if spectrum.meta is None:
            new_meta = {}
        else:
            new_meta = spectrum.meta
        ...
        binned_spectrum = sp.Spectrum1D(
            ...
            uncertainty=astropy.nddata.StdDevUncertainty(
                y_err.statistic[mask]),
            meta=new_meta.update(
...
def finder(array_original: np.ndarray,
           array_shifted: np.ndarray) -> np.ndarray:
    """
    Find indices in array_shifted that correspond to the closest matching values in array_original.

    Parameters
    ----------
    array_original : np.ndarray
        The reference array to match against, must be sorted in ascending order.
    array_shifted : np.ndarray
        The array containing values to be matched with array_original.

    Returns
    -------
    np.ndarray
        An array of indices from array_shifted where values most closely match 
        values in array_original. Only returns indices for values that fall within 
        the range of array_original.
    """
    duplicates = np.searchsorted(array_original, array_shifted)
    unique = np.unique(duplicates)
    unique = unique[unique < array_original.shape[0]]
    closest_indices = np.zeros(unique.shape[0])
    for idx, value in enumerate(unique):
        bw = np.argmin(np.abs(array_original[value] - array_shifted[duplicates == value]))
        tt = duplicates == value
        closest_indices[idx] = np.where(tt == True)[0][bw]
...
def _shift_spectrum(spectrum: sp.Spectrum1D | sp.SpectrumCollection,
    ...
    """
    if spectrum.spectral_axis is None:
        raise ValueError('The spectral axis is not defined.')
    else:
        if spectrum.spectral_axis.unit is None:
            raise ValueError('The spectral axis unit is not defined.')
    
    # If spectral axis is in velocity space
    if spectrum.spectral_axis.unit.is_equivalent(u.km / u.s): #type: ignore
        new_x_axis = spectrum.spectral_axis.to_value(u.km / u.s) #type: ignore
        for velocity in velocities:
            new_x_axis = new_x_axis + velocity.to(u.km / u.s).value #type: ignore
    # If spectral axis is in wavelength space
    elif spectrum.spectral_axis.unit.is_equivalent(u.AA): #type: ignore
        # Initiation of the term (1 since we are dividing/multiplying the term)
        term = 1
        # To factor all velocities (replace with relativistic one?)
        for velocity in velocities:
            # Divide as the velocities are in opposite sign
            term = term / (1 + velocity.to(u.m / u.s) / con.c) #type: ignore
        new_x_axis = spectrum.spectral_axis * term
    else:
        raise ValueError(
            f'The unit of velocities is wrong. The units of velocities is {[velocity.unit.is_equivalent(u.AA) for velocity in velocities]}') #type: ignore
    # Mask handling
    ...
    if spectrum.uncertainty is not None:
        ...
        mask = mask_flux
        raise ValueError('The uncertainty is not defined.')
    ...
    new_flux = interpolate_flux(spectrum.spectral_axis)

    import warnings
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        new_uncertainty = np.sqrt(interpolate_error(spectrum.spectral_axis))

    new_mask_flux = np.isnan(new_flux)
    new_mask_err = np.isnan(new_uncertainty)
    new_mask = np.logical_or(new_mask_flux, new_mask_err)

    new_uncertainty = astropy.nddata.StdDevUncertainty(
        new_uncertainty)  # Interpolate on the old wave_grid
    ...
    if isinstance(spectrum, sp.Spectrum1D):
        new_spectrum = sp.Spectrum1D(
            ...
            meta=spectrum.meta.copy() if spectrum.meta else {},
            mask=new_mask,
        )
    elif isinstance(spectrum, sp.SpectrumCollection):
        logger.warning(
            'Spectral Collection format has been untested, please verify')
        new_spectrum = sp.SpectrumCollection(
            spectral_axis=spectrum.spectral_axis,
            flux=new_flux * spectrum.flux.unit,
            uncertainty=new_uncertainty,
...
def mask_shift(new_x_axis: sp.SpectralAxis, mask: np.ndarray) -> sp.SpectralRegion:
    """
    Adjusts the mask to the new x-axis and creates a SpectralRegion object.

    Parameters:
    new_x_axis (Any): The new x-axis with bin edges.
    mask (np.ndarray): A boolean array indicating the mask.
    ...
    left_regions, right_regions = a[~test_mask], np.roll(
        np.roll(b, 1)[~test_mask], -1)

    masking_region = sp.SpectralRegion(left_regions[0], right_regions[0])
    for left, right in zip(left_regions[1:], right_regions[1:]):
...
def _interpolate2commonframe_1D(spectrum: sp.Spectrum1D,
    ...
    if len(spectrum.spectral_axis) == len(new_spectral_axis):
        if np.array_equal(spectrum.spectral_axis, new_spectral_axis):
            logger.debug(
                'Skipping interpolation as the desired spectral axis is the same as of the spectrum.')
            return spectrum
    ...
    # Interpolate on the old wave_grid
    new_flux = flux_interpolate(new_spectral_axis)

    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        # Interpolate on the old wave_grid
        new_uncertainty = np.sqrt(error_interpolate(new_spectral_axis))
    ...
    new_uncertainty = astropy.nddata.StdDevUncertainty(
        new_uncertainty * scaling_uncertainty)
...
def binning_list(spectrum_list: sp.SpectrumList,
    ...
    if force_multiprocessing:
        logger.warning(
            'Starting multiprocessing - interpolation to common grid')
        with Pool() as p:
            new_spectrum_list = p.starmap(interpolate2commonframe,
                                          zip(spectrum_list, repeat(
                                              new_spectral_axis))
                                          )
...
def median_subtract(spectrum_list: sp.SpectrumList):
    ...
    if type(spectrum_list[0]) == sp.Spectrum1D:
        median_spectrum = sp.Spectrum1D(
            ...
            flux=np.nanmedian(np.asarray(
                [spectrum.flux for spectrum in spectrum_list]), axis=0) * u.dimensionless_unscaled
        )
    ...
    for spectrum in spectrum_list:
        new_spectrum_list.append(
            spectrum.subtract(median_spectrum, handle_meta='first_found').add(
                1*u.dimensionless_unscaled, handle_meta='first_found')
        )
    return new_spectrum_list
...
def _normalize_spectrum_quantile(spectrum: sp.Spectrum1D | sp.SpectrumCollection,
    ...
    normalization_function = astropy.nddata.NDDataArray(
        data=normalization_function)

    match type(spectrum):
        case sp.Spectrum1D:
            new_spectrum = sp.Spectrum1D(
                spectral_axis=spectrum.spectral_axis,
                flux=old_flux.divide(
                    normalization_function).data * u.dimensionless_unscaled,
                # The numpy thing is weird, why does this work like this?
                uncertainty=astropy.nddata.StdDevUncertainty(old_flux.divide(
                    normalization_function).uncertainty.array.to_numpy()),
                mask=spectrum.mask if spectrum.mask is not None else None,
                meta=spectrum.meta if spectrum.meta is not None else {},
            )
        case sp.SpectrumCollection:
            raise NotImplementedError('Spectral collection are not used.')

    new_spectrum.meta['normalization'] = True # type: ignore
    return new_spectrum
...
def calculate_master_list(spectrum_list: sp.SpectrumList,
    ...
    if key is None:
        sublist = sp.SpectrumList(spectrum_list.copy())
    else:
        sublist = get_sublist(spectrum_list, key, value)
        spectrum_list = sublist

    assert (len(spectrum_list) !=
            0), 'The spectrum list is empty. Cannot calculate the master.'
    ...
    for ni in range(number_of_nights):
        ...
        if len(sublist) == 0:
            logger.warning(
                f'No spectra were found for given key/value selection for night {ni}. This night will be skipped and the resulting master list will not have a spectrum for this night.')
            continue
...
def wiggle_correction(spectrum_list: sp.SpectrumList,
    ...
    for ind, item in enumerate(spectrum_list):
        logger.info(
            f'Correcting for wiggles and renormalizing for spectrum {ind}/{len(spectrum_list)}')
...
def master_out_correction(spectrum_list: sp.SpectrumList,
    ...
    if unit is not None:
        ...
        if isinstance(corrected_list, sp.Spectrum1D) or isinstance(corrected_list, sp.SpectrumCollection):
            corrected_list = sp.SpectrumList([corrected_list])
...
def master_out_correction_StarRotator(spectrum_list: sp.SpectrumList,
    ...
    assert len(RM_model.spectra) == len(
        spectrum_list), 'RM model from StarRotator does not have same length as spectra'
    assert (RM_model.times == np.asarray([item.meta['Phase'].data for item in spectrum_list])).all(
    ), 'The phase array of RM model and spectrum list is not the same'

    corrected_list = sp.SpectrumList()

    if RM_model.residual is None or RM_model.lightcurve is None:
        raise ValueError("RM_model.residual and RM_model.lightcurve must not be None")
    for spectrum, RM_master, delta in zip(spectrum_list, RM_model.residual, RM_model.lightcurve):
        ...
        RM_residuals = sp.Spectrum1D(
            spectral_axis=RM_model.wl * 10 * u.AA, # type: ignore
            flux=RM_master/delta * u.dimensionless_unscaled,
            uncertainty=astropy.nddata.StdDevUncertainty(
                np.zeros_like(RM_master))
        )
        ...
        divided_spectrum = spectrum.divide(
            master_night, handle_meta='first_found')
...
def exciser_fill_with_nan(spectrum, region):
    ...
    for subregion in region:
        ...
        region_mask = (spectral_axis >= region.lower) & (
            spectral_axis < region.upper)
        region_mask = (spectral_axis >= subregion.lower) & (
            spectral_axis < subregion.upper)
...
def extract_velocity_field(spectrum: sp.Spectrum1D,
                           ...
                           shift_constant: u.Quantity = 0*u.km/u.s,#type: ignore
                           ) -> list[u.Quantity]:
    """
    ...
    ----------
    spectrum : sp.Spectrum1D
        Spectrum from which to extract velocities.
    shift_BERV : float
        1/0/-1, otherwise the velocity is scaled.
    shift_v_sys : float
        1/0/-1, otherwise the velocity is scaled.
    shift_v_star : float
        1/0/-1, otherwise the velocity is scaled.
    ...
    shift_constant : u.Quantity, optional
        Constant velocity shift, by default 0*u.km/u.s.

    Returns
    -------
    list[u.Quantity]
        List of velocities to shift by.
    """
    velocity_field = []
    
    if spectrum.meta is None:
        raise ValueError('The spectrum does not have meta data.')
    
    if shift_BERV != 0:
        velocity_field.append(
            spectrum.meta['velocity_BERV'].data * spectrum.meta['velocity_BERV'].unit * shift_BERV)
    if shift_v_sys != 0:
        velocity_field.append(
            spectrum.meta['velocity_system'].data * spectrum.meta['velocity_system'].unit * shift_v_sys)
    if shift_v_star != 0:
        velocity_field.append(
            spectrum.meta['velocity_star'].data * spectrum.meta['velocity_star'].unit * shift_v_star)
    if shift_v_planet != 0:
        velocity_field.append(
            spectrum.meta['velocity_planet'].data * spectrum.meta['velocity_planet'].unit * shift_v_planet)
    if shift_constant != 0:
...
def shift_list(spectrum_list: sp.SpectrumList,
    ...
    if force_multiprocessing:
        ...
        for ind, spectrum in enumerate(spectrum_list):
            logger.info(
                f'Shifting spectrum number: {ind+1}/{len(spectrum_list)}')
...
def extract_average_uncertainty_from_region(spec, line_list, diff_unc=1*u.AA):#type: ignore
    """
    ...
    for line in line_list:
        ...
        unc_list.append(np.nanmean(extract_region_in_spectrum(
            spec, spec_region).uncertainty.array))
    return unc_list
# %% velocity_domain_sgl
...
def velocity_domain_single_line(spectrum: sp.Spectrum1D | sp.SpectrumCollection,
                                line: u.Quantity,
                                constraint: list = [-100, 100]*u.km/u.s) -> sp.Spectrum1D | sp.SpectrumCollection:#type: ignore
    """
    ...
    if isinstance(spectrum, sp.Spectrum1D):
        velocity_spectrum = spectrum.with_spectral_axis_unit(
            u.km/u.s,#type: ignore
            velocity_convention='relativistic',
            rest_value=line
        )
    elif isinstance(spectrum, sp.SpectrumCollection):
        raise NotImplementedError("with_spectral_axis_unit is not available for SpectrumCollection")
    else:
        raise TypeError("Unsupported spectrum type")
...
def velocity_fold_single_spectrum(spectrum: sp.Spectrum1D | sp.SpectrumCollection,
                                  ...
                                  constraint: list = [-100, 100]*u.km/u.s #type: ignore
                                  ) -> tuple[sp.Spectrum1D | sp.SpectrumCollection, sp.SpectrumList]:
    """
...
def velocity_fold_spectrum_list(spectrum_list: sp.SpectrumList,
                                line_list: list[u.Quantity],
                                constraint: list = [-100, 100]*u.km/u.s #type: ignore
                                ) -> tuple[sp.SpectrumList, sp.SpectrumList]:
    """
    ...
    return sp.SpectrumList(new_spectrum_list), sp.SpectrumList(line_spectra_separated)
# %% Injection of planetary signal
def inject_planetary_signal(spectrum_list: sp.SpectrumList,
                            template: sp.Spectrum1D,
                            offset: u.Quantity = -100*u.km/u.s #type: ignore
                            ) -> sp.SpectrumList:
    """
        ...
        if item.meta['Transit_partial']:
            shifted_template = _shift_spectrum(
                ...
                velocities=extract_velocity_field(
                    ...
                    shift_v_star=0,
                    shift_v_planet=-1,
                    shift_constant= u.Quantity(-offset),
                )
            )
            ...
            new_spectrum_list.append(
                sp.Spectrum1D(
                    ...
                    flux=(item.flux.value *
                          shifted_template.flux.value) * item.flux.unit,
