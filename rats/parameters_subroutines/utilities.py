"""
This module provides utility functions for handling parameters and subroutines, including calculations for planetary properties and system parameters.

Functions:
- _Kepler_func(ecc_anom, mean_anom, ecc):
    Finds the Eccentric anomaly using the mean anomaly and eccentricity.

- CalculationPlanet:
    A class to calculate planetary properties.

- CalculationSystem:
    A class to calculate system properties.

- CalculationTransitLength:
    A class to calculate transit lengths.

- EquivalenciesTransmission:
    A class to define transmission spectrum specific units.

- StellarModel:
    A class to generate stellar models.

- LimbDarkening:
    A class to calculate limb darkening coefficients.

- ModellingLightCurve:
    A class to model light curves.
"""
...
class CalculationPlanet:
    ...
    def _calculate_gravity_acceleration(self):
        ...
        self.gravity_acceleration = self.gravity_acceleration.convert_unit_to(
            u.m/u.s/u.s)  # type:ignore
        self.gravity_acceleration.meta = {
            'parameter': 'Gravity acceleration',
            'Reference': 'calculated'
        }
        self.gravity_acceleration = ndutils._round_to_precision(
    ...
    def _calculate_atmospheric_scale_height(self, molecular_mass=2.33):
        ...
        try:
            ...
            logger.warning(
                'Gravity acceleration was not calculated before, recalculating.')
        ...
        self.atmospheric_scale_height = ndutils._round_to_precision(
            self.atmospheric_scale_height)
...
class CalculationSystem:
    ...
    def _calculate_planet_velocity(self,
        ...
        planet_velocity : NDDataArray
            Planet velocity
        """
        semimajor_axis = self.Planet.semimajor_axis.convert_unit_to(
            u.m)  # type:ignore
        period = self.Ephemeris.period.convert_unit_to(u.s)  # type:ignore
        inclination = self.Planet.inclination.convert_unit_to(
            u.rad)  # type:ignore
    ...
    def _calculate_stellar_velocity(self,
        ...
        if np.isnan(self.Planet.argument_of_periastron):  # type:ignore
            omega = NDDataArray(
                90*u.deg, uncertainty=StdDevUncertainty(0))  # type:ignore
            omega = omega.convert_unit_to(u.rad)
        else:
            omega = self.Planet.argument_of_periastron.convert_unit_to(
                u.rad)  # type:ignore

        keplerian_semiamplitude = self.Planet.keplerian_semiamplitude.convert_unit_to(
            u.m/u.s)  # type:ignore
        eccentricity = self.Planet.eccentricity  # type:ignore
        eccentricity = NDDataArray(eccentricity.data, uncertainty=StdDevUncertainty(
            eccentricity.uncertainty.array))
        ...
        return (self.Planet.semimajor_axis.divide(self.Star.radius).convert_unit_to(u.dimensionless_unscaled)).multiply(  # type:ignore
            (  # a/Rs * (sin(2pi * phase) * cos(lambda) + cos(2pi * phase) * cos(i) * sin(lambda)) * vsini
                (ndutils._sin_NDDataArray(phase.multiply(2*np.pi*u.rad))).multiply(
                    ndutils._cos_NDDataArray(
                        self.Planet.projected_obliquity)  # type:ignore
                ).add(
    ...
    def _calc_true_anom(self, phase: float | NDDataArray) -> NDDataArray:
        ...
        if math.isclose(eccentricity.data,
            ...
            sin_eccentricity_anomaly_TR = ndutils._sin_NDDataArray(
                eccentricity_anomaly_TR)
            eccentricity = NDDataArray(eccentricity.data, uncertainty=StdDevUncertainty(
                eccentricity.uncertainty.array))  # Removing unit
            ...
            mean_anomaly_TR = eccentricity_anomaly_TR.subtract(
                subtraction_term)

            # mean_anomaly_TR = ecc_anom_TR-sys_para.planet.e*np.sin(ecc_anom_TR)*u.rad

            if (mean_anomaly_TR.data < 0.):
                mean_anomaly_TR = mean_anomaly_TR.add(
                    NDDataArray(2.*np.pi*u.rad))

            # Mean anomaly
            #  - time origin of t_mean at the periapsis (t_mean=0 <-> M=0 <-> E=0)
            #  - M(t_mean)=M(dt_transit)+M(t_simu)
            mean_anomaly = NDDataArray(
                2*np.pi*u.rad).multiply(phase).add(mean_anomaly_TR)
            ...
            eccentricity_anomaly = NDDataArray(
                eccentricity_anomaly * u.rad, uncertainty=StdDevUncertainty(0))
    ...
    def _calculate_velocities_spectrum(self,
        ...
        assert ('Phase' in spectrum.meta.keys()
                ), 'Spectrum does not have "Phase" keyword.'
        assert not (
            spectrum.meta['Phase'] == 'undefined'), 'Spectrum "Phase" keyword is not defined.'

        velocity_planet = self._calculate_planet_velocity(
            spectrum.meta['Phase'])
        velocity_star = self._calculate_stellar_velocity(
            spectrum.meta['Phase'])
...
class CalculationTransitLength:
    ...
    def _update_contact_points(self,
        ...
        if self.Ephemeris.transit_length_partial is None or force_recalculate or np.isnan(self.Ephemeris.transit_length_partial.data):
            ...
            amplitude = self.Ephemeris.period.divide(
                (np.pi * u.rad))  # P/ pi #type:ignore
            scale = self.Star.radius.divide(
                self.Planet.semimajor_axis)  # R_star / a #type:ignore

            one = NDDataArray(1, unit=u.dimensionless_unscaled)

            nominator_unrooted_term1 = one.add(
                self.Ephemeris.transit_depth)  # (1+k) #type:ignore
            nominator_unrooted_term1 = ndutils._power_NDDataArray(
                nominator_unrooted_term1, 2)  # (1+k)**2
            nominator_unrooted_term2 = (self.Planet.impact_parameter.multiply(
                self.Planet.impact_parameter))  # b**2 #type:ignore
            nominator_unrooted = nominator_unrooted_term1.subtract(
                nominator_unrooted_term2)
            nominator_unrooted = nominator_unrooted.convert_unit_to(
                u.dimensionless_unscaled)

            nominator_squareroot = ndutils._power_NDDataArray(
                nominator_unrooted, power=(1/2))
            denominator = ndutils._sin_NDDataArray(
                self.Planet.inclination)  # type:ignore
            term_inside_sinus = scale.multiply(nominator_squareroot).divide(
                denominator).convert_unit_to(u.dimensionless_unscaled)
            ...
            eccentricity_denominator = (
                self.Planet.eccentricity.multiply(  # type:ignore
                    ndutils._sin_NDDataArray(
                        self.Planet.argument_of_periastron)  # type:ignore
                )
            ).add(one)

            eccentricity_correction = eccentricity_nominator.divide(
                eccentricity_denominator)
            ...
            self.Ephemeris.transit_length_partial = ndutils._round_to_precision(
                self.Ephemeris.transit_length_partial)  # type:ignore

            logger.info('(Re)-calculation of transit length')
            # type:ignore
            logger.info(
                f'    Partial transit (T14) length: {self.Ephemeris.transit_length_partial}')
            # type:ignore
            logger.info(
                f'recalculated based on: {self.Ephemeris.transit_length_partial.meta["formula"]}')
        ...
        if self.Ephemeris.transit_length_full is None or force_recalculate or np.isnan(self.Ephemeris.transit_length_full.data):
            ...
            nominator_unrooted_term1 = (one.subtract(
                self.Ephemeris.transit_depth))  # (1-k) #type:ignore
            nominator_unrooted_term1 = ndutils._power_NDDataArray(
                nominator_unrooted_term1, 2)  # (1-k)**2
            nominator_unrooted_term2 = (self.Planet.impact_parameter.multiply(
            ...
            self.Ephemeris.transit_length_full = ndutils._round_to_precision(
                self.Ephemeris.transit_length_full)  # type:ignore

            logger.info('(Re)-calculation of transit length')
            # type:ignore
            logger.info(
                f'    Full transit (T23) length: {self.Ephemeris.transit_length_full}')
            # type:ignore
            logger.info(
                f'recalculated based on: {self.Ephemeris.transit_length_full.meta["formula"]}')
    ...
    def _phase_fold(self,
        ...
        phase = NDDataArray(
            phase.data % 1, uncertainty=StdDevUncertainty(phase.uncertainty.array))
    ...
    def _calculate_contact_points(self):
        ...
        T14 = self.Ephemeris.transit_length_partial.convert_unit_to(
            u.h)  # type:ignore
        T23 = self.Ephemeris.transit_length_full.convert_unit_to(
            u.h)  # type:ignore
        P = self.Ephemeris.period.convert_unit_to(u.h)  # type:ignore

        self.Ephemeris.contact_T1 = T14.divide(P).divide(
            NDDataArray(-1)).divide(NDDataArray(2))  # type:ignore
        self.Ephemeris.contact_T2 = T23.divide(P).divide(
            NDDataArray(-1)).divide(NDDataArray(2))  # type:ignore
        self.Ephemeris.contact_T3 = T23.divide(
            P).divide(NDDataArray(2))  # type:ignore
        self.Ephemeris.contact_T4 = T14.divide(
            P).divide(NDDataArray(2))  # type:ignore
    ...
    def plot_contact_points(self,
        ...
        """
        ax.axhline(self.Ephemeris.contact_T1.data, ls=ls,
                   color=color_partial)  # type:ignore
        ax.axhline(self.Ephemeris.contact_T2.data, ls=ls,
                   color=color_full)  # type:ignore
        ax.axhline(self.Ephemeris.contact_T3.data, ls=ls,
                   color=color_full)  # type:ignore
        ax.axhline(self.Ephemeris.contact_T4.data, ls=ls,
                   color=color_partial)  # type:ignore
...
class EquivalenciesTransmission:
    ...
    def _custom_transmission_units(self):
        ...
        Rp, Rs = self.Planet.radius.data * self.Planet.radius.unit, self.Star.radius.data * \
            self.Star.radius.unit  # type:ignore
        H = self.Planet.atmospheric_scale_height.convert_unit_to(
            u.km).data  # In km #type:ignore
        rat = (Rs/Rp).decompose().value  # There is a rat in my code, oh no!
        Rp_km = Rp.to(u.km).value  # Planet radius in km #type:ignore

        # Definition of units
        self.unit_Transmission = u.def_unit(
            ['', 'T', 'Transmitance', 'Transmitted flux'])
        self.unit_Absorption = u.def_unit(
            ['', 'R', 'Excess atmospheric absorption', 'A'])
        self.unit_PlanetRadius = u.def_unit(['Rp', 'Planetary radius'])
        self.unit_TransitDepth = u.def_unit(
            ['', 'Wavelength dependent transit depth'])
        self.unit_NumberOfAtmosphericScaleHeights = u.def_unit(
...
class LimbDarkening:
    ...
    def calculate_limb_darkening_coefficients(self):
        ...
        # type:ignore
        logger.print(
            f"    {ldc['espresso_uniform.dat']['quadratic']['Merged']['ALL'][0][0]}")
        # type:ignore
        logger.print(
            f"    {ldc['espresso_uniform.dat']['quadratic']['Merged']['ALL'][1][0]}")
...
class ModellingLightCurve:
    ...
    def model_light_curve(self,
        ...
        # time of inferior conjunction #type:ignore
        params.t0 = self.Ephemeris.transit_center.data
        params.per = self.Ephemeris.period.data  # orbital period #type:ignore
        # planet radius (in units of stellar radii) #type:ignore
        params.rp = self.Planet.rprs.data
        # semi-major axis (in units of stellar radii) #type:ignore
        params.a = self.Planet.a_rs_ratio.data
        # orbital inclination (in degrees) #type:ignore
        params.inc = self.Planet.inclination.data
        ...
        # longitude of periastron (in degrees) #type:ignore
        params.w = argument_of_periastron
        # limb darkening coefficients [u1, u2] #type:ignore
        params.u = [u1, u2]
    ...
    def add_transit_depth_value(self,
        ...
        t = np.asarray(
            [spectrum.meta['BJD'].value for spectrum in spectrum_list])
        lc_model = batman.TransitModel(self.lc_params, t)  # initializes model
        transit_depth = lc_model.light_curve(
            self.lc_params)  # calculates light curve
